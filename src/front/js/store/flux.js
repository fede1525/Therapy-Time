		...
		actions: {
			...
			createUser: async (body) => {
				try {
					...
					const resp = await protectedFetch("/signup", "POST", body);
			
					if (resp.error) {
						throw new Error(resp.error || "Error al crear el usuario.");
					}
			...
			getUsers: async () => {
				try {
					const resp = await protectedFetch("/users", "GET");
					if (resp.error) {
			...
			getUser: async (id) => {
				try {
					const resp = await protectedFetch(`/get_user/${id}`, "GET");
					if (resp.error) {
			...
			editUser: async (id, userData) => {
				try {
					const resp = await protectedFetch(`/edit_user/${id}`, "PUT", userData);
					if (resp.error) {
